...
class UpdateOrderItemRequestTest extends TestCase
{
  ...
  public function test_authorize_returns_true_when_user_can_update_item()
  {
    ...
    $user->shouldReceive('can')->with('updateItem', Mockery::type(\App\Models\Order::class))->andReturn(true);
  ...
  public function test_authorize_returns_false_when_user_cannot_update_item()
  {
    DB::table('users')->insert(['id' => 1, 'name' => 'Test User', 'email' => 'test@example.com', 'password' => 'password', 'role' => 'user', 'status' => 'active']);
    DB::table('clients')->insert([
      'id' => 1,
    ...
    $user = Mockery::mock(User::class);
    $user->shouldReceive('can')->with('updateItem', Mockery::type(\App\Models\Order::class))->andReturn(false);
