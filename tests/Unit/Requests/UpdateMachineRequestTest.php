...
class UpdateMachineRequestTest extends TestCase
{
  use RefreshDatabase;
  ...
  protected function setUp(): void
  {
    ...
    $this->userWithPermission = User::factory()->create([
      'role' => 'user',
      'permissions' => ['machines' => ['update' => true]]
    ]);
    $this->userWithoutPermission = User::factory()->create([
      'role' => 'user',
      'permissions' => ['machines' => ['update' => false]]
    ]);
  ...
  public function test_admin_can_authorize_update_request()
  {
    ...
    $request = new UpdateMachineRequest();
    $request->setUserResolver(fn() => $this->admin);
    $request->setRouteResolver(fn() => $this->createMockRoute($this->machine));
  ...
  public function test_validation_rules_require_unique_name_per_sector_ignoring_current_machine()
  {
    Auth::login($this->admin);
    ...
    $this->assertStringContainsString('ignore(' . $this->machine->id, $rules['name'][4]);
  }
  ...
  public function test_validation_allows_same_name_for_same_machine()
  {
    Auth::login($this->admin);
  ...
  private function createMockRoute($machine)
  {
    $route = $this->createMock(\Illuminate\Routing\Route::class);
    $route->method('parameter')->willReturn($machine);
    return $route;
  }
}
