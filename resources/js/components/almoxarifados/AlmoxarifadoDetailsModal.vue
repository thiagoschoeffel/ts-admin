...
const props = defineProps({
  ...
  almoxarifadoId: { type: [Number, String, null], default: null },
});
...
const payload = ref(null); // structured almoxarifado data (required)

watch(() => props.modelValue, (v) => { open.value = v; if (v) tryFetch(); });
watch(open, (v) => emit('update:modelValue', v));
watch(() => props.almoxarifadoId, () => { if (open.value) tryFetch(); });

async function tryFetch() {
  if (!props.almoxarifadoId) return;
  loading.value = true;
  error.value = false;
  payload.value = null;
  try {
    const res = await fetch(route('almoxarifados.modal', props.almoxarifadoId), {
      headers: { Accept: 'application/json' },
      credentials: 'same-origin',
    });
    if (!res.ok) throw new Error('failed');
    const data = await res.json();
    if (!data || !data.almoxarifado) throw new Error('invalid');
    payload.value = data.almoxarifado;
  } catch (_) {
    error.value = true;
  } finally {
    loading.value = false;
...
<template>
  <Modal v-model="open" title="Detalhes do almoxarifado" size="lg" :lockScroll="true" :closeOnBackdrop="true">
    <div v-if="loading" class="space-y-6" aria-hidden="true">
      ...
      <span class="sr-only">Carregando detalhes do almoxarifado...</span>
    </div>

    <div v-else-if="error" class="flex flex-col items-center justify-center gap-3 text-center text-sm text-slate-500">
      <p class="text-sm text-rose-600">Não foi possível carregar os detalhes do almoxarifado.</p>
