...
watch(() => props.productionPointingId, () => { if (open.value) tryFetch(); });
...
async function tryFetch() {
  ...
  payload.value = null;
  try {
    const res = await fetch(route('production-pointings.modal', props.productionPointingId), {
      headers: { Accept: 'application/json' },
      credentials: 'same-origin',
    });
    if (!res.ok) throw new Error('failed');
    const data = await res.json();
    if (!data || !data.productionPointing) throw new Error('invalid');
    payload.value = data.productionPointing;
  } catch (_) {
    error.value = true;
  } finally {
    loading.value = false;
...
const operatorNames = computed(() => payload.value?.operators?.map((o) => o.name) ?? []);
const siloNames = computed(() => payload.value?.silos?.map((s) => s.name) ?? []);
</script>

<template>
  <Modal v-model="open" title="Detalhes do apontamento de produção" size="lg" :lockScroll="true" :closeOnBackdrop="true">
    <div v-if="loading" class="space-y-6" aria-hidden="true">
      ...
      <span class="sr-only">Carregando detalhes do apontamento de produção...</span>
    </div>

    <div v-else-if="error" class="flex flex-col items-center justify-center gap-3 text-center text-sm text-slate-500">
      <p class="text-sm text-rose-600">Não foi possível carregar os detalhes do apontamento.</p>
    ...
    <div v-if="payload" class="space-y-6">
      <section class="space-y-3">
        ...
        <dl class="grid gap-4 sm:grid-cols-2">
          <div class="space-y-1">
            <dt class="text-sm font-semibold text-slate-500">Número da ficha</dt>
            <dd class="text-sm text-slate-800">{{ payload.sheet_label ?? payload.sheet_number ?? '—' }}</dd>
          </div>
          <div class="space-y-1">
          ...
          <div class="space-y-1">
            ...
            <dd class="text-sm text-slate-800">{{ payload.raw_material ?? '—' }}</dd>
          </div>
          <div class="space-y-1">
            <dt class="text-sm font-semibold text-slate-500">Quantidade (kg)</dt>
            <dd class="text-sm text-slate-800">{{ formatQuantity(payload.quantity) }}</dd>
          </div>
          <div class="space-y-1">
      ...
      <section class="space-y-3">
        <h2 class="text-lg font-semibold text-slate-900">Equipe e recursos</h2>
        <dl class="grid gap-4 sm:grid-cols-2">
          <div class="space-y-1">
            <dt class="text-sm font-semibold text-slate-500">Operadores</dt>
            <dd class="text-sm text-slate-800">
              <span v-if="operatorNames.length === 0">—</span>
              <ul v-else class="list-disc list-inside space-y-1">
                <li v-for="name in operatorNames" :key="name">{{ name }}</li>
              </ul>
            </dd>
          </div>
          <div class="space-y-1">
            <dt class="text-sm font-semibold text-slate-500">Silos</dt>
            <dd class="text-sm text-slate-800">
              <span v-if="siloNames.length === 0">—</span>
              <ul v-else class="list-disc list-inside space-y-1">
                <li v-for="name in siloNames" :key="name">{{ name }}</li>
              </ul>
            </dd>
          </div>
        </dl>
