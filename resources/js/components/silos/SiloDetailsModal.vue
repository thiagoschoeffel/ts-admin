...
const props = defineProps({
  ...
  siloId: { type: [Number, String, null], default: null },
});
...
const payload = ref(null); // structured silo data (required)

watch(() => props.modelValue, (v) => { open.value = v; if (v) tryFetch(); });
watch(open, (v) => emit('update:modelValue', v));
watch(() => props.siloId, () => { if (open.value) tryFetch(); });

async function tryFetch() {
  if (!props.siloId) return;
  loading.value = true;
  error.value = false;
  payload.value = null;
  try {
    const res = await fetch(route('silos.modal', props.siloId), {
      headers: { Accept: 'application/json' },
      credentials: 'same-origin',
    });
    if (!res.ok) throw new Error('failed');
    const data = await res.json();
    if (!data || !data.silo) throw new Error('invalid');
    payload.value = data.silo;
  } catch (_) {
    error.value = true;
  } finally {
    loading.value = false;
...
<template>
  <Modal v-model="open" title="Detalhes do silo" size="lg" :lockScroll="true" :closeOnBackdrop="true">
    <div v-if="loading" class="space-y-6" aria-hidden="true">
      ...
      <span class="sr-only">Carregando detalhes do silo...</span>
    </div>

    <div v-else-if="error" class="flex flex-col items-center justify-center gap-3 text-center text-sm text-slate-500">
      <p class="text-sm text-rose-600">Não foi possível carregar os detalhes do silo.</p>
