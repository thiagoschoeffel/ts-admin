...
const props = defineProps({
  ...
  rawMaterialId: { type: [Number, String, null], default: null },
});
...
const payload = ref(null); // structured rawMaterial data (required)

watch(() => props.modelValue, (v) => { open.value = v; if (v) tryFetch(); });
watch(open, (v) => emit('update:modelValue', v));
watch(() => props.rawMaterialId, () => { if (open.value) tryFetch(); });
...
async function tryFetch() {
  ...
  payload.value = null;
  try {
    const res = await fetch(route('raw-materials.modal', props.rawMaterialId), {
      headers: { Accept: 'application/json' },
      credentials: 'same-origin',
    });
    if (!res.ok) throw new Error('failed');
    const data = await res.json();
    if (!data || !data.rawMaterial) throw new Error('invalid');
    payload.value = data.rawMaterial;
  } catch (_) {
    error.value = true;
  } finally {
    loading.value = false;
...
<template>
  <Modal v-model="open" title="Detalhes da matéria-prima" size="lg" :lockScroll="true" :closeOnBackdrop="true">
    <div v-if="loading" class="space-y-6" aria-hidden="true">
      ...
      <span class="sr-only">Carregando detalhes da matéria-prima...</span>
    </div>

    <div v-else-if="error" class="flex flex-col items-center justify-center gap-3 text-center text-sm text-slate-500">
      <p class="text-sm text-rose-600">Não foi possível carregar os detalhes da matéria-prima.</p>
