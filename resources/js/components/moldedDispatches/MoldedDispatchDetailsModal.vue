...
const props = defineProps({
  ...
  moldedDispatchId: { type: [Number, String, null], default: null },
});
...
watch(() => props.moldedDispatchId, () => { if (open.value) tryFetch(); });

async function tryFetch() {
  if (!props.moldedDispatchId) return;
  loading.value = true;
  error.value = false;
  payload.value = null;
  try {
    const res = await fetch(route('molded-dispatches.modal', props.moldedDispatchId), {
      headers: { Accept: 'application/json' },
      credentials: 'same-origin',
    });
    if (!res.ok) throw new Error('failed');
    const data = await res.json();
    if (!data || !data.moldedDispatch) throw new Error('invalid');
    payload.value = data.moldedDispatch;
  } catch (_) {
    error.value = true;
  } finally {
    loading.value = false;
...
<template>
  <Modal v-model="open" title="Detalhes da saída de moldados" size="lg" :lockScroll="true" :closeOnBackdrop="true">
    <div v-if="loading" class="space-y-6" aria-hidden="true">
      ...
      <span class="sr-only">Carregando detalhes da saída de moldados...</span>
    </div>
    ...
    <div v-if="payload" class="space-y-6">
      <section class="space-y-3">
        ...
        <dl class="grid gap-4 sm:grid-cols-2">
          ...
          <div class="space-y-1">
            <dt class="text-sm font-semibold text-slate-500">Tipo de moldado</dt>
            <dd class="text-sm text-slate-800">{{ payload.mold_type_name ?? '—' }}</dd>
          </div>
          <div class="space-y-1">
            <dt class="text-sm font-semibold text-slate-500">Quantidade</dt>
            <dd class="text-sm text-slate-800">{{ payload.quantity ?? 0 }}</dd>
          </div>
        </dl>
      </section>
