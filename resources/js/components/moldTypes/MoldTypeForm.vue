...
<template>
  <form @submit.prevent="$emit('submit')" class="space-y-6">
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <label class="form-label">
        Quantidade de peças por pacote (UND) *
        <InputNumber v-model="form.pieces_per_package" :formatted="true" :precision="2" :min="0.01" :step="0.01" placeholder="0,00" required :error="!!form.errors.pieces_per_package" />
        <span v-if="form.errors.pieces_per_package" class="text-sm font-medium text-rose-600">{{ form.errors.pieces_per_package }}</span>
      </label>

      <div class="switch-field sm:col-span-2 lg:col-span-3">
        <span class="switch-label">Status do tipo de moldado</span>
