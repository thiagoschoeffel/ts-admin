...
const form = useForm({
  ...
  production_pointing_id: null,
  block_production_ids: [],
});
...
async function loadBlocks(ppId) {
  if (!ppId) {
    resetBlocksState();
    return;
  }
  ...
  try {
    const { data } = await axios.get(route('block-dispatches.available-blocks'), {
      params: { production_pointing_id: ppId },
    });
...
watch(
  ...
  (val) => {
    ...
    debounceTimer = setTimeout(() => loadBlocks(val), 350);
  }
);
...
function submit() {
  form.post(route('block-dispatches.store'));
}
...
<template>
  <AdminLayout>
    <Head title="Nova Saída de Blocos" />
      ...
      <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 class="text-2xl font-semibold text-slate-900 flex items-center gap-2">
            ...
            Nova saída de blocos
          </h1>
          <p class="mt-2 text-sm text-slate-500">Registre a saída (baixa) de blocos por requisição.</p>
        </div>
      </div>
